    ...
    */
    ...
    {    
        ...
        if(rank==0)
        {
            ...
            for(int e=1;e<=num_bodies;e++)
            {
                MPI_Recv(&array[e]->part->x,2,MPI_DOUBLE,e,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            }
            for(int u=1;u<=num_bodies;u++)
            {

                //cout<<setprecision(20)<<a[u]->x<<" "<<a[u]->y<<" "<<a[u]->vel->x<<" "<<a[u]->vel->y<<" "<<a[u]->acc->x<<" "<<a[u]->acc->y<<endl<<endl;
                cout<<setprecision(20)<<array[u]->part->x<<"      "<<array[u]->part->y<<" "<<endl;
            }
            gettimeofday(&t1, 0);
        ...
        else
        {
            ...
            force(array[rank],ce,&force_x[rank],&force_y[rank]);
        ...
        //cout<<endl<<endl<<endl;
            ...
            newax=force_x[rank]/array[rank]->part->mass;
            neway=force_y[rank]/array[rank]->part->mass;
            array[rank]->part->acc->x+=newax;
            array[rank]->part->acc->y+=neway;
            array[rank]->part->vel->x+=(array[rank]->part->acc->x)*dt;
            array[rank]->part->vel->y+=(array[rank]->part->acc->y)*dt;
            array[rank]->part->x+=array[rank]->part->vel->x*dt;
            array[rank]->part->y+=array[rank]->part->vel->y*dt;
            //cout<<array[rank]->part->x<<endl;
            //MPI_Send(&array[rank]->part->x,1,MPI_DOUBLE,0,0,MPI_COMM_WORLD);
            //cout<<array[rank]->part->x<<endl;
            ...
            MPI_Send(&array[rank]->part->x,2,MPI_DOUBLE,0,0,MPI_COMM_WORLD);
        }
    }
}
